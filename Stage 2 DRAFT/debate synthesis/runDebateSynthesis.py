...
def _is_section_complete(target_company: str) -> bool:
    """Predicate for section_runner: synthesis content populated. The structured
    fields (synthesis_score / categorical / confidence) are best-effort —
    debateSynthesis.py logs a warning and saves the narrative when JSON parsing
    fails, so the section is "done" once `synthesis` is non-empty even if the
    score is None."""
    ...
    try:
        ...
        return False
    return bool(data.get("synthesis"))
