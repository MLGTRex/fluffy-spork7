...
try:
    ...
    # Optional imports — surfaced as API failure if missing at call time
    pass
...
# ============ MOONSHOT TOOL CHAT CLIENT ============
# Same pattern as the hourly watcher's llmTriggerClassifier.

def _normalise_formula_uri(uri: str) -> str:
    if "/" not in uri:
        uri = f"moonshot/{uri}"
    if ":" not in uri:
        uri = f"{uri}:latest"
...
def _looks_like_text_tool_call(content: str) -> bool:
    """
    Detect a Moonshot tool-call directive emitted as plain text rather than via
    the structured tool_calls API field. This happens when the model still
    wants a tool after the tool list has been withdrawn. A real investigation
    answer never contains these markers.
    """
    if not content:
        return False
...
class _FormulaChatClient:
    """Thin client over Moonshot's chat + formula tools."""
    ...
    def __init__(self, base_url: str, api_key: str, model: str, max_tokens: int,
        ...
        self.token_usage = {"input_tokens": None, "output_tokens": None,
                            "total_tokens": None}
    ...
    async def handle_response(self, response, messages, all_tools, tool_to_uri):
        """Recursive handler. Base case: text response. Recursive: tool calls."""
        ...
        if not message.tool_calls:
            if (_looks_like_text_tool_call(message.content)
                    and self.force_answer_attempts < MAX_FORCE_ANSWER_ATTEMPTS):
                self.force_answer_attempts += 1
                logger.warning(
                    "[LLM] Model emitted a tool call as plain text instead of a "
                    f"final answer; re-prompting for the JSON (attempt "
                    f"{self.force_answer_attempts}/{MAX_FORCE_ANSWER_ATTEMPTS})."
                )
                messages.append({"role": "user", "content": FORCE_ANSWER_INSTRUCTION})
            ...
            logger.info("[LLM] Final text response generated.")
        ...
        if self.tool_call_count >= self.max_tool_calls:
            ...
            logger.warning(
                f"Web-search cap of {self.max_tool_calls} reached "
                f"(actual: {self.tool_call_count}). Forcing final answer."
            )
            next_tools = None
            # Tell the model explicitly to stop searching and emit the JSON, so
            # it does not reply with a text <tool> directive instead.
            messages.append({"role": "user", "content": FORCE_ANSWER_INSTRUCTION})
...
async def investigate_ticker(
    ...
    *,
    ...
    try:
        uri = _normalise_formula_uri(formula)
        try:
